...
<tr>
<td align="left" style="padding:15px;Margin:0"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px">Hi <b>${userName}</b></p></td>
</tr>
<tr>
...
export async function sendPostTaskEmail(emailParams) {
  try {
    await sgMail.send(getMessage(emailParams));
    return  { message: `Task notification is sent successfully for: ${emailParams.taskName}`};
  } catch (error) {
    const message = `Error in sending task notification for: ${emailParams.taskName}`;
